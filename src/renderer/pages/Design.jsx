...
export default function Design() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Stay on the Design page — reset the form so the operator can keep
      // uploading the next design. The Convert tab then builds _qr for it.
      const sid = res.data.order?.system_id || res.data.order?.id;
      notify(`Design created: ${sid}. Sang tab Convert _qr để build.`, { title: 'Design created', kind: 'success' });
      setForm({
        method: 'label',
        ref_id: '',
        shipping_label: '',
      ...
      notify(msg, { title: 'Create design failed', kind: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6">
      <h2 className="text-xl font-bold text-neutral-800 mb-1">Design — Upload & Create</h2>
      <p className="text-xs text-neutral-500 mb-4">Upload ảnh design (front/back/metas) → tạo → sang tab Convert _qr để build, rồi Gangsheet để gộp.</p>
